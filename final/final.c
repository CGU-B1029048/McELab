...
const unsigned char pacman_right[2][16] = 
{{0x00, 0x00, 0xE0, 0xF8, 0xF8, 0xFC, 0xFE, 0xFE, 0xFE, 0xFE, 0x66, 0x34, 0x1C, 0x08, 0x00, 0x00},
{0x00, 0x00, 0x07, 0x1F, 0x1F, 0x3F, 0x7F, 0x7F, 0x7F, 0x7F, 0x7E, 0x3C, 0x38, 0x10, 0x00, 0x00}};

const unsigned char pacman_down[2][16] = 
{{0x00, 0x80, 0xE0, 0xF0, 0xF8, 0xF8, 0xFC, 0xFC, 0xFC, 0xFC, 0xF8, 0xF8, 0xF0, 0xE0, 0x80, 0x00},
{0x00, 0x03, 0x1F, 0x3F, 0x1F, 0x0F, 0x07, 0x03, 0x03, 0x07, 0x0F, 0x13, 0x31, 0x1F, 0x03, 0x00}};

const unsigned char pacman_up[2][16] = 
...
{
	const unsigned char (*pacman)[16];
	int i, x;
	Set_DisplayStartLine (z_in);
	if(status == 1) pacman = pacman_right;
	if(status == 3) pacman = pacman_up;
	if(status == 4) pacman = pacman_down;
	for (x = 0; x < 2; x++) {
		Set_Xaddr (x);

		if (mode && y_in < 64) { //draw left
			Set_Yaddr (y_in);
			for (i=0;i < ((y_in > 48) ? 64 - y_in : 16);i++)
				(status == 2)? Send_Data (pacman[x][15-i]):Send_Data (pacman[x][i]);
		}

		if (!mode && y_in > 48) { //draw right
			Set_Yaddr((y_in < 64)? 0: y_in-64);
			for (i = ((y_in > 63) ? 0 : 64 - y_in);i < 16;i++)
				(status == 2)? Send_Data (pacman[x][15-i]):Send_Data (pacman[x][i]);
		}
	}
}
